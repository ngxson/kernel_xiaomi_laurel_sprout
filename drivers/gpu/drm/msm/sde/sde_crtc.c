...
static int _xiaomi_alpha_lookup_tlb[] = { 255, 232, 224, 218, 212, 208, 204,
	200, 197, 194, 191, 188, 185, 183, 180, 178, 175, 173, 171, 169, 167, 165,
	163, 161, 159, 158, 156, 154, 153, 151, 149, 148, 146, 145, 143, 142, 140,
	139, 137, 136, 135, 133, 132, 131, 129, 128, 127, 125, 124, 123, 122, 121,
	119, 118, 117, 116, 115, 114, 112, 111, 110, 109, 108, 139, 138, 137, 136,
	135, 134, 134, 133, 132, 131, 130, 130, 129, 128, 127, 127, 126, 125, 124,
	124, 123, 122, 122, 121, 120, 119, 119, 118, 117, 117, 116, 115, 115, 114,
	113, 113, 112, 111, 111, 110, 109, 109, 108, 108, 107, 106, 106, 105, 104,
	104, 103, 103, 102, 101, 101, 100, 100, 99, 98, 98, 97, 97, 96, 95, 95, 94,
	94, 93, 93, 92, 92, 91, 90, 90, 89, 89, 88, 88, 87, 87, 86, 86, 85, 84, 84,
	83, 83, 82, 82, 81, 81, 80, 80, 79, 79, 78, 78, 77, 77, 76, 76, 75, 75, 74,
	74, 73, 73, 72, 72, 71, 71, 70, 70, 69, 69, 68, 68, 67, 67, 67, 66, 66, 65,
	65, 64, 64, 63, 63, 62, 62, 61, 61, 61, 60, 60, 59, 59, 58, 58, 57, 57, 57,
	56, 56, 55, 55, 54, 54, 53, 53, 53, 52, 52, 51, 51, 50, 50, 50, 49, 49, 48,
	48, 47, 47, 47, 46, 46, 45, 45, 45, 44, 44, 43, 43, 43, 42, 42, 41, 41, 41,
	40, 40, 39, 39, 39, 38, 38, 37, 37, 37, 36, 36, 35 };
static int _xiaomi_fod_get_dim_alpha(int brightness) {
	brightness = (brightness < 0) ? 0 : brightness;
	brightness = (brightness > 255) ? 255 : brightness;
	return _xiaomi_alpha_lookup_tlb[brightness];
}

static void _xiaomi_fod_setup_dim_layer(struct sde_crtc_state *cstate, struct plane_state *pstates, int cnt) {
	struct sde_hw_dim_layer *fingerprint_dim_layer;
	int zpos = 0;
	int i;
	int alpha;

	if (_xiaomi_fod_enable) {
		for (i = 0; i < cnt; i++) {
			if (pstates[i].stage > zpos) zpos = pstates[i].stage;
		}
		zpos++;
		alpha = _xiaomi_fod_get_dim_alpha(_xiaomi_bl_lvl);
		fingerprint_dim_layer = &cstate->dim_layer[cstate->num_dim_layers];
		fingerprint_dim_layer->flags = SDE_DRM_DIM_LAYER_EXCLUSIVE;
		fingerprint_dim_layer->stage = zpos + SDE_STAGE_0;
		fingerprint_dim_layer->rect.x = 293;    // sensorLocationX
		fingerprint_dim_layer->rect.y = 1356;   // sensorLocationY
		fingerprint_dim_layer->rect.w = 134;    // sensorDiameter
		fingerprint_dim_layer->rect.h = 134;    // sensorDiameter
...
#ifdef CONFIG_MACH_XIAOMI_F9S
	_xiaomi_fod_setup_dim_layer(cstate, pstates, cnt);
#endif
